...
export default function AlertsPage() {
  ...
  const canCreateAlert = hasPermission(user, "createAlert");
  ...
  return <section className="content-section alerts-workspace">
    ...
    {assets.length > 0 && canCreateAlert && <CreateAlertForm assets={assets} onCreated={addAlert} />}
    <section className="alert-register"><div className="alert-register-heading"><div><p className="section-kicker">Alert register</p><h3>{filteredAlerts.length} matching {filteredAlerts.length === 1 ? "alert" : "alerts"}</h3></div><div className="filter-status">{loadState === "loading" ? "Updating register" : "Live register"}</div></div>
      ...
      {loadState !== "loading" && filteredAlerts.length === 0 && <div className="detail-state"><strong>{alerts.length === 0 ? "No alerts recorded" : "No matching alerts"}</strong><p>{alerts.length === 0 ? (canCreateAlert ? "Create an alert when an equipment signal needs review." : "There are no alert records to review.") : "Adjust the filters to see other records."}</p></div>}
