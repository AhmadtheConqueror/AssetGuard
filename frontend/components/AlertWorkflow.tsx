...
export function AlertList({
  ...
  return <div className="alert-record-list">{alerts.map((alert) => {
    ...
    return <article className="alert-record" key={alert.id}>
      ...
      <div className="alert-record-body"><p>{alert.description || "No description provided."}</p><dl className="alert-record-facts"><div><dt>Source</dt><dd>{alert.source === "condition_monitoring" ? "Condition Monitoring" : "Manual"}</dd></div><div><dt>Detected</dt><dd>{formatDateTime(alert.detected_at)}</dd></div>{alert.condition_assessment_evaluated_at && <div><dt>Triggered by assessment</dt><dd>{formatDateTime(alert.condition_assessment_evaluated_at)}</dd></div>}{alert.ai_escalation_status && <div><dt>AI enrichment</dt><dd>{alert.ai_escalation_status}</dd></div>}{alert.acknowledged_at && <div><dt>Acknowledged</dt><dd>{formatDateTime(alert.acknowledged_at)}</dd></div>}{alert.resolved_at && <div><dt>Resolved</dt><dd>{formatDateTime(alert.resolved_at)}</dd></div>}</dl></div>
